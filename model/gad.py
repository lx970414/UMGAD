...
theta=1.001,eta=1.001):
    ...
    anomaly_scores= model.decision_function(graph)
    edge_anomaly_scores=model.decision_struct_function(graph)

    auc_score = eval_roc_auc(graph.y.bool().cpu().numpy(), anomaly_scores)

    print(f'auc_score: {auc_score:.4f}',)

    return auc_score,None,None,None,None,anomaly_scores,edge_anomaly_scores
